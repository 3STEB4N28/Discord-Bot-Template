...
try{
    ...
    ['command'].forEach(handler=>{//REDIRECTS TO COMMAND HANDLER
        require(`./handler/command`)(Bot);
    })
    console.log(`[>] Loaded a total of ${Bot.commands.cache.size} Commands`)
        ...
        mongoose.connect(config.MONGODB_URL,{//Connecting to MongoDB
            ...
            useUnifiedTopology:true,
                ...
                id:message.guild.id
                    ...
                    if(err){
                        return console.log(`[>] Cannot connect to DataBase!`);//Logs if cannot connect to DB
                    }else if(!err){
                            ...
                            Bot.on(`ready`, async () =>{
                                console.log(
                                    ...
                                    Bot.user.setPresence({ status: 'online' });//Changing its Discord Presence (Making it look Online)
                                        ...
                                        Bot.user.setActivity(`in ${Bot.guilds.cache.size} Servers with ${Bot.users.cache.size} Users!`, {type: activities_list[index].type,}//Setting the Bot Status
                                        );
                            })
